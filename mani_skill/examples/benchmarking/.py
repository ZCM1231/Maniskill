...
NUM_ENVS_LIST = [4, 8, 16, 32, 64, 128, 256, 512]
@dataclass
class Args:
    ...
    save_results: Optional[str] = "/home/zcm/lightrobot/ManiSkill/mani_skill/examples/benchmarking"
...
def main(args: Args):
    for num_envs in NUM_ENVS_LIST:
        for env_id in BENCHMARK_ENVS:
            print(f"Running benchmark for env_id: {env_id}, num_envs: {num_envs}")
            ...
            if not args.cpu_sim:
                ...
                env = gym.make_vec(env_id, num_envs=num_envs, vectorization_mode="async", vector_kwargs=dict(context="spawn"),
                                   obs_mode=args.obs_mode, )
                base_env = gym.make(env_id, obs_mode=args.obs_mode).unwrapped
            ...
            with torch.inference_mode():
                ...
                if args.save_video:
                    ...
                    images_to_video(
                        ...
                        video_name=f"mani_skill_gpu_sim-random_actions-{env_id}-num_envs={num_envs}-obs_mode={args.obs_mode}-render_mode={args.render_mode}",
                        fps=30,
                    )
                    del images
                        ...
                        if v["control_mode"] == "pd_joint_pos":
                            ...
                            if args.save_video:
                                ...
                                images_to_video(
                                    images,
                                    output_dir="./videos/ms3_benchmark",
                                    video_name=f"mani_skill_gpu_sim-fixed_trajectory={k}-{env_id}-num_envs={num_envs}-obs_mode={args.obs_mode}-render_mode={args.render_mode}",
                                    fps=30,
                                )
                                del images
                env.reset(seed=2022)
                ...
                if args.save_example_image:
                    ...
                    for cam_name, cam_data in obs["sensor_data"].items():
                        for k, v in cam_data.items():
                            imgs = v.cpu().numpy()
                            imgs = tile_images(imgs, nrows=int(np.sqrt(num_envs)))
                            cmap = None
                            if k == "depth":
                                imgs[imgs == np.inf] = 0
                                imgs = imgs[:, :, 0]
